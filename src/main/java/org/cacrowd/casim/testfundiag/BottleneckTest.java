...
public class BottleneckTest {
	...
	public static void main (String [] args){
		...
		for (float w = tic; Math.round(w*10)/10f<=maxWidth; w+=tic){
			...
			BottleneckTestRunner runner = new BottleneckTestRunner(700, 20.0f, 16.0f, w, 1.2f, 11.8f);
	...
	private static void setupCommonConstants() {
		org.cacrowd.casim.pedca.utility.Constants.RANDOM_SEED = new Random().nextLong();
		Constants.SIMULATION_DURATION = 3000;
		org.cacrowd.casim.pedca.utility.Constants.DENSITY_GRID_RADIUS = 1.2;
		Constants.ORIGIN_FLOWS[0] = "s";
		Constants.FLOPW_CAP_PER_METER_WIDTH = 20000.;
		Constants.VIS = true;
	}
