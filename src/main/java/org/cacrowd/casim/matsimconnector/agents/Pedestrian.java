...
public class Pedestrian extends Agent {
	...
	public Pedestrian(int Id, GridPoint position, Destination destination, Context context, QVehicle vehicle, TransitionArea transitionArea, String envCAId){
	...
	@Override
	public void percept(double time){
		if (destinationReached && transitionArea!=null && getStaticFFValue(getPosition())==0.){
			exit();
		}
		if (timeToCrossDestination > 0) --timeToCrossDestination;
		if (waitingTime > 0) --waitingTime;
		else if (destination instanceof ScheduledDestination)
			this.waitingTime = ((ScheduledDestination)destination).waitingTimeForCrossing(time);
	...
	private void perceptIfFinalDestination(double now) {
		if (timeToCrossDestination == 0 && getStaticFFValue(getPosition())==0. && transitionArea == null){
			...
			if (now < Constants.CA_FD_TEST_END_TIME){
	...
	private void generateId(String envCAId){
		Id = IdUtility.createPedestrianId(envCAId, super.getID());
	}
	...
	public void refreshDestination(double time) {
	...
	public Neighbourhood getNeighbourhood(){
		...
		if(stopOnStairs() || timeToCrossDestination > 0 || isWaiting()){
	...
	private boolean isWaiting(){
		return this.waitingTime > 0 && CASimRandom.nextDouble() > .25;
	}
